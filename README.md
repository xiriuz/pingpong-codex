...
왼손과 라켓을 든 오른손이 모두 보이는 귀여운 1인칭 모바일 탁구 게임입니다. 왼손을 위로 올려 서브하고 오른손을 위·아래로 스윙하며, 뾰족머리와 포니테일 중 캐릭터를 고를 수 있습니다. 혼자 AI와 놀거나 한 폰을 가로로 나눠 친구와 함께 플레이할 수 있습니다.

## 플레이

**[GitHub Pages에서 바로 하기](https://xiriuz.github.io/pingpong-codex/)**
...
1. `혼자 하기` 또는 `둘이 하기`를 누릅니다.
2. 캐릭터를 누르면 게임 화면이 바로 열립니다.
3. 왼쪽 화면을 위로 밀어 왼손으로 공을 올린 뒤, 오른쪽 화면을 위·아래로 휘두르면 서브가 시작됩니다.
4. 오른손을 위로 휘두르면 톱스핀, 아래로 휘두르면 백스핀이 걸립니다. 사이드스핀은 사용하지 않습니다.
...
현재 `boy_idle.png`와 `girl_idle.png`는 최초 손그림을 참조해 색연필 질감과 캐릭터 모습을 유지한 에셋입니다.
...
- 서브는 자기 코트와 상대 코트에 차례로 한 번씩 바운드하며, 랠리는 상대 상판을 목표로 낮게 날아갑니다.
- 조작은 Pointer Events와 `pointerId`로 분리해 최대 4손가락 동시 입력을 지원합니다.
- 전체 게임 시간 배율은 `config.js`의 `GAME_SPEED`로 조절하며 현재는 원래 속도의 약 1/3입니다.
