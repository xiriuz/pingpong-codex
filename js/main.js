...
function frame(now){requestAnimationFrame(frame);const elapsed=Math.min(.1,(now-last)/1000);last=now;if(!state){drawTitleBackdrop(now/1000);return;}if(state.paused||orientationBlocked){render(ctx,innerWidth,innerHeight,state,assets,input);return;}accumulator+=elapsed;let steps=0;const events=[];while(accumulator>=C.FIXED_DT&&steps<C.MAX_STEPS){const inputs=controllers.map(controller=>controller.update(state,C.FIXED_DT));stepPhysics(state,inputs,C.FIXED_DT,events);updateEffects(state,C.FIXED_DT);accumulator-=C.FIXED_DT;steps++;}if(steps===C.MAX_STEPS)accumulator=0;for(const e of events)processEvent(e);ui.serve(!state.ball.active&&state.status==='playing'&&state.pointDelay<=0);render(ctx,innerWidth,innerHeight,state,assets,input);}
