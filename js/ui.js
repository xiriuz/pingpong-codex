...
export class UI{
  ...
  serve(show,stage,firstPoint){const guide=document.querySelector('#serve-guide'),help=document.querySelector('#start-help');guide.classList.toggle('show',show);guide.textContent=stage===0?'③ 왼손을 위로 올려 공을 띄워!':'좋아! 오른손으로 위·아래 스윙! 🏓';help.classList.toggle('show',show&&firstPoint);help.classList.toggle('swing-stage',stage===1);help.querySelector('.help-move strong').textContent=stage===0?'위로 올려 서브!':'공이 올라갔어!';help.querySelector('.help-hit strong').textContent=stage===0?'기다렸다가 스윙!':'위·아래로 스윙!';}
  orientation(block){document.body.classList.toggle('versus-portrait',block);}
  result(state){const win=state.players[state.winner],lost=state.players[1-state.winner],name=id=>id==='boy'?'뾰족머리':'포니테일';document.querySelector('#result-title').textContent=state.winner===0?'이겼어!':'우와, 멋진 경기!';document.querySelector('#result-message').textContent=state.mode==='single'?(state.winner===0?'정말 잘했어! 반짝반짝 최고야!':'아깝다! 한 번 더 하면 더 잘할 거야!'):`${name(win.character)} 최고! ${name(lost.character)}도 잘했어!`;document.querySelector('#best-rally').textContent=state.bestRally;const c=document.querySelector('#winner'),ctx=c.getContext('2d'),img=document.querySelector(`[data-character="${win.character}"] img`);ctx.clearRect(0,0,c.width,c.height);if(img?.complete&&!img.hidden)ctx.drawImage(img,42,5,176,250);else drawCharacter(ctx,win.character,130,145,190,'win');this.show('result-screen');}
  ...
  drawCards(){document.querySelectorAll('.character-card').forEach(card=>{const c=card.querySelector('canvas'),ctx=c.getContext('2d'),img=card.querySelector('img');drawCharacter(ctx,card.dataset.character,100,116,130,'win');img.onload=()=>{c.style.display='none';};img.onerror=()=>{img.style.display='none';};});}
